...
class ExchangeDialog(ui.ScriptWindow):
	...
	def __init__(self):
		...
		if app.WJ_ENABLE_TRADABLE_ICON:
			...
			if app.ENABLE_SPECIAL_INVENTORY:
				self.lockedItems = {i:(player.INVENTORY_TYPE_INVENTORY, -1,-1) for i in range(exchange.EXCHANGE_ITEM_MAX_NUM)}
			else:
	...
	def LoadDialog(self):
		...
		if app.ENABLE_RENDER_TARGET_SYSTEM:
			self.OwnerSlot.SetUnselectItemSlotEvent(ui.__mem_func__(self.UseItemSlotTarget))
		...
		if app.ENABLE_RENDER_TARGET_SYSTEM:
			self.TargetSlot.SetUnselectItemSlotEvent(ui.__mem_func__(self.UseItemSlotOwner))
	...
	def CloseDialog(self):
		...
		if app.WJ_ENABLE_TRADABLE_ICON:
			if app.ENABLE_SPECIAL_INVENTORY:
				for exchangePos, (invenType, itemInvenPage, itemSlotPos) in self.lockedItems.items():
					if invenType == self.wndInventory.GetInventoryType() and itemInvenPage == self.wndInventory.GetInventoryPageIndex():
	...
	def Refresh(self):
		...
		if exchange.GetAcceptFromSelf() == True:
			if not app.ENABLE_NEW_EXCHANGE_WINDOW:
				...
				self.OwnerSlot.SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 0.3500, 0.8500, 0.3500, 1.0)
			...
			if not app.ENABLE_NEW_EXCHANGE_WINDOW:
				...
				self.OwnerSlot.SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 1.0, 1.0, 1.0, 1.0)
		...
		if exchange.GetAcceptFromTarget() == True:
			if not app.ENABLE_NEW_EXCHANGE_WINDOW:
				...
				self.TargetSlot.SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 0.3500, 0.8500, 0.3500, 1.0)
			...
			if not app.ENABLE_NEW_EXCHANGE_WINDOW:
				...
				self.TargetAcceptButton.SetUp()
				self.TargetSlot.SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 1.0, 1.0, 1.0, 1.0)
		if app.WJ_ENABLE_TRADABLE_ICON:
			self.RefreshLockedSlot()

	if app.ENABLE_RENDER_TARGET_SYSTEM:
		def UseItemSlotTarget(self, slotIndex):
			if app.IsPressed(app.DIK_LSHIFT):
				itemVnum = exchange.GetItemVnumFromSelf(slotIndex)
				self.tooltipItem.ModelPreviewFull(itemVnum)	

		def UseItemSlotOwner(self, slotIndex):
			if app.IsPressed(app.DIK_LSHIFT):
