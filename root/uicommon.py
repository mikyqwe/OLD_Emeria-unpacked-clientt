...
if app.ENABLE_NEW_PET_SYSTEM:
	class PetTypePopupDialog(ui.ScriptWindow):
		...
		def SetText(self, text, withEnter = False):
			if withEnter == True:
				self.board.SetSize(320, 125)
				self.message.SetPosition(0, 30)
				self.accceptButton.SetPosition(0, 80)
			self.message.SetEnterToken(withEnter)
			self.message.SetText(text)
