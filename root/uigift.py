...
class GiftDialog(ui.ScriptWindow):
	...
	def LoadWindow(self):
		...
		self.UI["slots"].SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 1.0, 1.0, 1.0, 1.0)
	...
	if app.ENABLE_CHANGELOOK_SYSTEM:
		def GetItemTransmutation(self,slot):
			try:
				return int(constInfo.gift_items[int(slot)]["transmutation"])
			except KeyError:
				return 0
	...
	def Refresh(self):
		for i in xrange(self.UI["slots"].GetSlotCount()):
			...
			if app.ENABLE_CHANGELOOK_SYSTEM:
				self.UI["slots"].SetCoverButton(i, "d:/ymir work/ui/game/quest/slot_button_00.sub", "d:/ymir work/ui/game/quest/slot_button_00.sub", "d:/ymir work/ui/game/quest/slot_button_00.sub", "icon/item/ingame_convert_mark.tga", FALSE, FALSE)
				itemTransmutation = self.GetItemTransmutation(i)
				if itemTransmutation > 0:
					self.UI["slots"].DisableCoverButton(i)
				else:
					self.UI["slots"].EnableCoverButton(i)
		...
		if self.tooltipItem:
			...
			if int(slot) in constInfo.gift_items.keys():
				...
				if it["vnum"]==1:
					...
					if app.ENABLE_CHANGELOOK_SYSTEM:
						itemTransmutation = self.GetItemTransmutation(slot)
						if itemTransmutation > 0:
							item.SelectItem(itemTransmutation)
							nameTransmutation = item.GetItemName()
							self.tooltipItem.AddItemData(int(it["vnum"]),it["sockets"],it["attrs"], 0, 0, nameTransmutation)
						else:
