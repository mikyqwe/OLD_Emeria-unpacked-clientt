...
class PetFeedWindow(ui.ScriptWindow):
	...
	def SelectEmptySlot(self, selectedSlotPos):
		...
		if mouseModule.mouseController.isAttached():
			...
			if attachedItemCount > 1:
				chat.AppendChat(chat.CHAT_TYPE_INFO, "Please read the description of your item!")
				return
			if attachedItemCount == 1:
				attachedItemCount = 0
	...
	def SendPetItem(self):
		...
		net.SendChatPacket("/feedcubepet %d" % (constInfo.FEEDWINDOW))
		for x in range(len(self.arryfeed)):
			self.arryfeed[x] = -1
			self.petslot.ClearSlot(x)
			self.petslot.RefreshSlot()
