...
class RefineDialog(ui.ScriptWindow):
	...
	def Open(self, scrollItemPos, targetItemPos):
		...
		self.toolTip.AddItemData(itemIndex, metinSlot, preview = 0)
...
class RefineDialogNew(ui.ScriptWindow):
	...
	def __LoadScript(self):
		...
		if app.ENABLE_REFINE_RENEWAL:
			...
			self.checkBox.SetTextInfo("Leave window open")
			self.checkBox.Show()
		self.isLoaded = TRUE
	...
	def __MakeSlot(self):
		slot = ui.ImageBox()
		slot.LoadImage("d:/ymir work/slot_lager/slot.tga")
		slot.Show()
	...
	def UpdateDialog(self):
		...
		newHeight = self.dialogHeight + 93
