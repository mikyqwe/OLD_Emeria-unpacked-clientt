...
class MainStream(object):
	...
	#	try:
	#		import introLoading1
	#		self.SetPhaseWindow(introLoading1.LoadingWindow(self))
	#	except:
	#		import exception
	#		exception.Abort("networkModule.SetLoadingPhase")
		...
		try:					
			class LoadingWindow:
				def __init__(self, stream):		
					...
					for x in xrange(2):
						self.LoadingImage.AppendImage("d:/ymir work/Loading/Loading_%d.png" % (x))
						#self.LoadingImage.SetCenterPosition()
						self.LoadingImage.SetPosition(wndMgr.GetScreenWidth() / 2 - 200, wndMgr.GetScreenHeight() / 2 - 178)
	...
	def SetLoginInfo(self, id, pwd, pin):
		...
		self.pin = pin
		import constInfo
		net.SetLoginInfo(id, pwd, pin, constInfo.SEL_LANG)
