...
#		self.ctrlShadowQuality.SetEvent(ui.__mem_func__(self.OnChangeShadowQuality))
		...
		if app.ENABLE_FOV_OPTION:
			self.systemWindowTextLineFovMode = ui.TextLine()
			self.systemWindowTextLineFovMode.SetParent(self)
			self.systemWindowTextLineFovMode.SetPosition(30, 210)
			self.systemWindowTextLineFovMode.SetText(uiScriptLocale.OPTION_FOV)
			self.systemWindowTextLineFovMode.Show()
			
			self.systemWindowFovModeSlider = ui.SliderBar()
			self.systemWindowFovModeSlider.SetParent(self)
			self.systemWindowFovModeSlider.SetPosition(110, 210)
			self.systemWindowFovModeSlider.SetSliderPos(float(systemSetting.GetFOVLevel()) / 90.0)
			self.systemWindowFovModeSlider.SetEvent(ui.__mem_func__(self.OnChangeFOV))
			self.systemWindowFovModeSlider.Show()
	...
	if app.ENABLE_FOV_OPTION:
		def OnChangeFOV(self):
			pos = self.systemWindowFovModeSlider.GetSliderPos()
			systemSetting.SetFOVLevel(int(pos / 0.011))
