...
class PetSystemIncubator(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		try:
			...
			pyScrLoader.LoadScriptFile(self, "apollo_scripts/PetHatchingWindow.py")
		except:
			import exception
	...
	def LoadPetIncubatorImg(self, new_pet):
		petarryname = ["Monkey hatch!", "Spider hatch!", "Razador hatch!", "Nemere hatch!", "Nessi hatch!"]
		petarryimg = [55701, 55702, 55703, 55704, 55705]
		chat.AppendChat(chat.CHAT_TYPE_INFO, "[Pet-System] "+petarryname[int(new_pet)]+".")
	...
	def RequestHatching(self):
		if self.petname.GetText() == "" or len(self.petname.GetText()) < 4:
			chat.AppendChat(chat.CHAT_TYPE_INFO, "[Pet-System] The pet name is not valid.")
			return
		...
		chat.AppendChat(chat.CHAT_TYPE_INFO, "[Pet-System]Your pet hatched.")
		import chr
