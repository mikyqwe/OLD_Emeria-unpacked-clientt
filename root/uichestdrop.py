...
class ChestDropWindow(ui.ScriptWindow):
	...
	def __LoadWindow(self):
		...
		wndItem.RefreshSlot()
		wndItem.SetSlotBaseImage("d:/ymir work/slot_lager/slot.tga", 1.0, 1.0, 1.0, 1.0)
		wndItem.Show()
	...
	def SetOpenAmount(self, amount):
		...
		self.openChestButtonSingle.SetText("Open %d chests" % self.openAmount)
		self.openChestButtonMultiple.SetText("Open %d chests" % self.openAmount)
	...
	def SetInvItemSlot(self, invItemPos):
		...
		self.RefreshInvItemSlot()
			
	def RefreshInvItemSlot(self):
		itemVnum = player.GetItemIndex(self.invItemPos)
		itemCount = player.GetItemCount(self.invItemPos)
	...
	def RefreshItems(self, chestVnum):
		...
		if not self.chestDrop.has_key(self.currentChest):
			chat.AppendChat(chat.CHAT_TYPE_INFO, "This chest has no drops. Please report this problem to an administrator.")
			self.Close()
			return
