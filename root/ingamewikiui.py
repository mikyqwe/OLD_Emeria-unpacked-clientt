...
class WikiScrollBar(ui.Window):
	...
	CORNERS_AND_LINES_COLOR = grp.GenerateColor(0.2156, 0.1058, 0.1058, 1.0)
	
	BAR_NUMB = 9 #This is static value! Please dont touch in him.
	
	class MiddleBar(ui.DragButton):
		MIDDLE_BAR_COLOR = grp.GenerateColor(0.7803, 0.6627, 0.4392, 1.0)
...
class WikiCategory(ui.Window):
	...
	LINES_COLOR = grp.GenerateColor(0.2156, 0.1058, 0.1058, 1.0)
	CORNER_COLOR = grp.GenerateColor(0.2156, 0.1058, 0.1058, 1.0)
	BASE_COLOR = grp.GenerateColor(0.0, 0.0, 0.0, 1.0)
...
class WikiMainWeaponWindow(ui.Window):
	...
	CLASS_BUTTONS = [
		[
			...
			classButtonsPath + "class_w_normal.png",
			classButtonsPath + "class_w_hover.png",
			classButtonsPath + "class_w_selected.png"
		],
		[
			item.ITEM_ANTIFLAG_ASSASSIN,
			classButtonsPath + "class_n_normal.png",
			classButtonsPath + "class_n_hover.png",
			classButtonsPath + "class_n_selected.png"
		],
		[
			item.ITEM_ANTIFLAG_SHAMAN,
			classButtonsPath + "class_s_normal.png",
			classButtonsPath + "class_s_hover.png",
			classButtonsPath + "class_s_selected.png"
		],
		[
			item.ITEM_ANTIFLAG_SURA,
			classButtonsPath + "class_su_normal.png",
			classButtonsPath + "class_su_hover.png",
			classButtonsPath + "class_su_selected.png"
		]
	]
...
class SimpleTextLoader(ui.Window):
	...
	def LoadFile(self, filename):
		...
		loadF = pack_open(open_filename, 'r')
...
class WikiMainCostumeWindow(ui.Window):
	...
	def OnOverIn(self, vnum, metinSlot = [0 for i in xrange(player.METIN_SOCKET_MAX_NUM)]):
		...
		if itemType == item.ITEM_TYPE_WEAPON or itemType == item.ITEM_TYPE_COSTUME and subType == item.COSTUME_TYPE_WEAPON:
			...
			self.wikiRenderTarget.SetModel(item.GetValue(3))
...
def InitMainWindow(self):
	...
	for i in inGameWikiConfig.MOB_BLACKLIST:
		if type(i) == types.ListType:
			for k in i:
...
def InitWindowAdds(self):
	self.add_left = ui.ExpandedImageBox()
	self.add_left.SetParent(self.baseBoard)
	#self.add_left.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/Decoration_bottom_left.png")
	self.add_left.SetPosition(-10, self.baseBoard.GetHeight() + 1 - 27)
	self.add_left.Show()
	
	self.add_right = ui.ExpandedImageBox()
	self.add_right.SetParent(self.baseBoard)
	#self.add_right.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/Decoration_bottom_right.png")
	self.add_right.SetPosition(self.baseBoard.GetWidth()+1 - 32, self.baseBoard.GetHeight() + 1 - 27)
	self.add_right.Show()
...
def InitTitleBar(self):
	...
	self.titleBar.SetWidth(self.GetWidth() - 7)
	self.titleBar.SetPosition(8-4, 4)
...
def BuildSearchWindow(self):
	...
	self.searchButton.SetUpVisual("d:/ymir work/ui/wiki/button_search_normal.png")
	self.searchButton.SetOverVisual("d:/ymir work/ui/wiki/button_search_hover.png")
	self.searchButton.SetDownVisual("d:/ymir work/ui/wiki/button_search_down.png")
	self.searchButton.SetPosition(self.searchBG.GetWidth() - self.searchButton.GetWidth() - 2 + 2, self.searchBG.GetHeight() / 2 - self.searchButton.GetHeight() / 2)
...
def BuildBaseMain(self):
	...
	self.category_bg = ui.ExpandedImageBox()
	self.category_bg.SetParent(self.baseBoard)
	self.category_bg.SetPosition(13, self.searchBG.GetLocalPosition()[1] + self.searchBG.GetHeight() + 25)
	self.category_bg.LoadImage("d:/ymir work/ui/wiki/category_bg.png")
	self.category_bg.Show()
	
	self.categText = ui.TextLine()
	self.categText.SetParent(self.category_bg)
	self.categText.SetPosition(5, 1)
	...
	self.prevButt.SetUpVisual("d:/ymir work/ui/wiki/btn_arrow_left_normal.png")
	self.prevButt.SetOverVisual("d:/ymir work/ui/wiki/btn_arrow_left_hover.png")
	self.prevButt.SetDownVisual("d:/ymir work/ui/wiki/btn_arrow_left_down.png")
	self.prevButt.SetDisableVisual("d:/ymir work/ui/wiki/btn_arrow_left_down.png")
	...
	self.nextButt.SetUpVisual("d:/ymir work/ui/wiki/btn_arrow_right_normal.png")
	self.nextButt.SetOverVisual("d:/ymir work/ui/wiki/btn_arrow_right_hover.png")
	self.nextButt.SetDownVisual("d:/ymir work/ui/wiki/btn_arrow_right_down.png")
	self.nextButt.SetDisableVisual("d:/ymir work/ui/wiki/btn_arrow_right_down.png")
	self.nextButt.SetPosition(self.categ.GetLocalPosition()[0] + self.categ.GetWidth() - self.nextButt.GetWidth() + 13, self.categ.GetLocalPosition()[1] + self.categ.GetHeight() + 5)
