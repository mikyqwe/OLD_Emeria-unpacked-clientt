...
class MiniMap(ui.ScriptWindow):
	...
	def __init__(self):
		...
		self.tooltipBioOpen = MapTextToolTip()
		#self.tooltipBioOpen.SetText("Biologist")
		self.tooltipBioOpen.Show()
		
		self.tooltipTeleportOpen = MapTextToolTip()
		self.tooltipTeleportOpen.SetText("Teleportpannel")
		self.tooltipTeleportOpen.Show()
	...
	def __Initialize(self):
		...
		self.BioShowButton = 0
		self.TeleportShowButton = 0
		...
		self.tooltipBioOpen = 0
		self.tooltipTeleportOpen = 0
		self.tooltipInfo = None
		self.serverInfo = None
		self.MiniMapWindow = None
	...
	def LoadWindow(self):
		...
		try:
			...
			#self.BioButton = self.GetChild("BioButton")
		...
		##self.BioButton.SetEvent(ui.__mem_func__(self.BioPort))
		self.TeleportButton.SetEvent(ui.__mem_func__(self.OpenTeleportPanel))
		...
		##(ButtonPosX, ButtonPosY) = self.BioButton.GetGlobalPosition()
		self.tooltipBioOpen.SetTooltipPosition(ButtonPosX, ButtonPosY)
		
		(ButtonPosX, ButtonPosY) = self.TeleportButton.GetGlobalPosition()
		self.tooltipTeleportOpen.SetTooltipPosition(ButtonPosX, ButtonPosY)
	...
	def OnUpdate(self):
		...
		if self.MiniMapShowButton and TRUE == self.MiniMapShowButton.IsIn():
			self.tooltipMiniMapOpen.Show()
		else:
			if self.tooltipMiniMapOpen:
		...
		#if TRUE == self.BioButton.IsIn():
			#self.tooltipBioOpen.Show()
		#else:
			#self.tooltipBioOpen.Hide()
			
		if TRUE == self.TeleportButton.IsIn():
			self.tooltipTeleportOpen.Show()
		else:
			self.tooltipTeleportOpen.Hide()
	...
	def OpenTeleportPanel(self):
		net.SendChatPacket("/teleportpanel_open")
	...
	if app.ENABLE_BIYOLOG:
		def OpenBio(self):
			try:
				...
				if interface:
					if interface.wndBio.IsShow():
						interface.wndBio.Hide()
					else:
						interface.wndBio.Show()
			except:
				pass