...
class Window(object):
	...
	def LoadPosition(self):
		self.SetPosition(self.baseX, self.baseY)
	...
	def IsInPositionNew(self, xMouse, yMouse):
		x, y = self.GetGlobalPosition()
		return xMouse >= x and xMouse < x + self.GetWidth() and yMouse >= y and yMouse < y + self.GetHeight()
...
class TextLine(Window):
	def __init__(self, name="NONAME_TextLine"):
		Window.__init__(self)
		self.max = 0
	...
	def ShowPercentage(self, xStartPct, xEndPct):
		wndMgr.SetRenderingRect(self.hWnd, xStartPct, 0.0, xEndPct, 0.0)
...
class MultiTextLine(Window):
	...
	def NewTextLine(self):
		line = TextLine()
		line.SetPackedFontColor(self.FONT_COLOR)
		line.SetParent(self)
...
#	
#			def SetStringEvent(self, event, func,isa=FALSE):
#				self.eventDict[event]=func
#	
...
####Item level text/icon ###
		if ItemIndex < 28000 or ItemIndex > 28943:
...
class TitleBar(Window):
	...
	def MakeTitleBar(self, width, color):
		...
		imgLeft.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/titlebar_left.png")
		imgCenter.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/titlebar_center.png")
		imgRight.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/titlebar_right.png")
		imgLeft.Show()
		imgRight.Show()
		imgCenter.Show()
		...
		cBtnBG.LoadImage(apollo_interface.PATCH_COMMON + "/titlebar/decoration_right.png")
		cBtnBG.SetParent(self)
		cBtnBG.Show()

		btnClose = Button()
		btnClose.SetParent(self)
		btnClose.SetUpVisual(apollo_interface.PATCH_COMMON + "/button/board_close_01_normal.png")
		btnClose.SetOverVisual(apollo_interface.PATCH_COMMON + "/button/board_close_02_hover.png")
		btnClose.SetDownVisual(apollo_interface.PATCH_COMMON + "/button/board_close_03_active.png")
	...
	def SetWidth(self, width):
		...
		self.btnClose.SetPosition(width - self.btnClose.GetWidth() +2, -2)
		self.SetSize(width, self.BLOCK_HEIGHT)

		self.cBtnBG.SetPosition(width - 45, -20)
...
class HorizontalSeparator(Window):
	...
	def __CreateUI(self):
		...
		self.__dictImages['LEFT'].LoadImage("d:/ymir work/horizontal_buttons/horizontal_left.tga")
		self.__dictImages['CENTER'].LoadImage("d:/ymir work/horizontal_buttons/horizontal_center.tga")
		self.__dictImages['RIGHT'].LoadImage("d:/ymir work/horizontal_buttons/horizontal_right.tga")
...
class Board(Window):
	...
	def __init__(self):
		...
		self.MakeBoard(apollo_interface.PATCH_COMMON + "/board/Corner_", apollo_interface.PATCH_COMMON + "/board/Bar_")
		self.MakeBase()
	...
	def MakeBase(self):
		...
		self.Base.LoadImage(apollo_interface.PATCH_COMMON + "/board/fill.png")
...
class BoardStyle2(Window): # anadir separator (common/separator)
	...
	def __init__(self):
		...
		self.MakeBoard("d:/ymir work/interface/board/Board_Corner_", "d:/ymir work/interface/Board/Board_Line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage("d:/ymir work/interface/Board/Board_Base.tga")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
		...
		self.decoration.LoadImage("d:/ymir work/interface/Board/decoration_leftbottom.tga")
...
class Board2(Window):
	...
	def __init__(self):
		...
		self.MakeBoard(apollo_interface.PATCH_COMMON + "/board2/board_Corner_", apollo_interface.PATCH_COMMON + "/board2/board_line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage(apollo_interface.PATCH_COMMON + "/board2/fill.png")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class BoardShop(Window):
	...
	def __init__(self, Path="d:/ymir work/ui/pattern/board"):
		Window.__init__(self)
		self.MakeBoard(Path)
		self.MakeBase(Path)
	...
	def MakeBoard(self, Path):
		...
		CornerFileNames = [ Path+"_corner_"+dir+".tga" for dir in ("LeftTop", "LeftBottom", "RightTop", "RightBottom", ) ]
		LineFileNames = [ Path+"_line_"+dir+".tga" for dir in ("Left", "Right", "Top", "Bottom", ) ]
	...
	def MakeBase(self, Path):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage(Path+"_base.tga")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class BoardWithTitleBarNew(BoardShop):
	def __init__(self, Path = "d:/ymir work/ui/pattern/board/" ):
		BoardShop.__init__(self, Path)
		path = Path
		titleBar = TitleBar()
		titleBar.SetParent(self)
		titleBar.MakeTitleBar(0, "red")
	...
	def SetSize(self, width, height):
		self.titleBar.SetWidth(width - 15)
		#self.pickRestrictWindow.SetSize(width, height - 30)
		BoardShop.SetSize(self,width, height)
		self.titleName.UpdateRect()
...
class NewImageBox(Window):
	...
	elif not app.ENABLE_SHOP_SEARCH_SYSTEM :
		def SAFE_SetStringEvent(self, event, func,isa=False):
			if not isa:
				self.eventDict[event]=__mem_func__(func)
			else:
...
class ThinBoardCircleNoAlpha(Window):
	...
	def __init__(self, layer = "UI"):
		...
		CornerFileNames = [ "d:/ymir work/ui/pattern/thinboardcircle2/ThinBoard_Corner_"+dir+"_circle.tga" for dir in ["LeftTop","LeftBottom","RightTop","RightBottom"] ]
		LineFileNames = [ "d:/ymir work/ui/pattern/thinboardcircle2/ThinBoard_Line_"+dir+"_circle.tga" for dir in ["Left","Right","Top","Bottom"] ]
...
class BorderA(Window):
	...
	def __init__(self, layer = "UI"):
		...
		#BaseFileName = "d:/ymir work/"+constInfo.GetUI()+"/pattern/border_a/border_center.tga"
...
class ThinBoardRoundCorner(Window):
	CORNER_WIDTH = 16
	CORNER_HEIGHT = 16
	LINE_WIDTH = 16
	LINE_HEIGHT = 16
	...
	def __init__(self, layer = "UI"):
		Window.__init__(self, layer)
		CornerFileNames = [ "d:/ymir work/ui/pattern/thinboard_r_corner/thinboard_corner_"+dir+".tga" for dir in ["lefttop", "leftbottom","righttop", "rightbottom"]]
		LineFileNames = [ "d:/ymir work/ui/pattern/thinboard_r_corner/thinboard_line_"+dir+".tga" for dir in ["left", "right", "top", "bottom"]]
...
class ThinScrollBar(ScrollBar):
	...
	def CreateScrollBar(self):
		...
		middleBar.SetMoveEvent(__mem_func__(self.OnMove))
		middleBar.Show()
		middleBar.SetUpVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_thinboard_middle_01_normal.png")
		middleBar.SetOverVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_thinboard_middle_02_hover.png")
		middleBar.SetDownVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_thinboard_middle_03_active.png")

		upButton = Button()
		upButton.SetParent(self)
		upButton.SetUpVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_up_01_normal.png")
		upButton.SetOverVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_up_02_hover.png")
		upButton.SetDownVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_up_03_active.png")
		upButton.SetEvent(__mem_func__(self.OnUp))
		upButton.Show()

		downButton = Button()
		downButton.SetParent(self)
		downButton.SetUpVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_down_01_normal.png")
		downButton.SetOverVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_down_02_hover.png")
		downButton.SetDownVisual(apollo_interface.PATCH_COMMON + "/scroll_bar/btn_down_03_active.png")
		downButton.SetEvent(__mem_func__(self.OnDown))
		downButton.Show()
...
class SliderBar(Window):
	...
	def __CreateBackGroundImage(self):
		img = ImageBox()
		img.SetParent(self)
		img.LoadImage(apollo_interface.PATCH_COMMON + "/slider/slider.png")
		img.Show()
	...
	def __CreateCursor(self):
		...
		cursor.SetUpVisual(apollo_interface.PATCH_COMMON + "/slider/btn_01_normal.png")
		cursor.SetOverVisual(apollo_interface.PATCH_COMMON + "/slider/btn_02_hover.png")
		cursor.SetDownVisual(apollo_interface.PATCH_COMMON + "/slider/btn_03_active.png")
		cursor.Show()
		self.cursor = cursor
...
class ListBoxEx_dd(Window):
	...
	def SetButtons(self, i, name_monster, iProb, Active, vnum, count, name_item):
		...
		self.text3[i] = MakeText(self.add[i], "- Monsters On Map: " + str(Active), 5, 100)
...
class ThinBoard(Window):
	...
	def __init__(self, layer = "UI"):
		...
		CornerFileNames = [ apollo_interface.PATCH_COMMON + "/thinboard_transparent/Corner_"+dir+".png" for dir in ["LeftTop", "LeftBottom","RightTop", "RightBottom"]]
		LineFileNames = [   apollo_interface.PATCH_COMMON + "/thinboard_transparent/bar_"+dir+".png" for dir in ["Left", "Right", "Top", "Bottom"]]
...
class ThinBoardGold(Window):
	...
	def __init__(self, layer = "UI"):
		Window.__init__(self, layer)
		CornerFileNames = [ apollo_interface.PATCH_COMMON + "/thinboard_intransparent/Corner_"+dir+".png" for dir in ["LeftTop", "LeftBottom","RightTop", "RightBottom"]]
		LineFileNames = [   apollo_interface.PATCH_COMMON + "/thinboard_intransparent/bar_"+dir+".png" for dir in ["Left", "Right", "Top", "Bottom"]]
		...
		Base.AddFlag("attach")
		Base.AddFlag("not_pick")
		Base.LoadImage(apollo_interface.PATCH_COMMON + "/thinboard_intransparent/fill.png")
		Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		Base.Show()
		self.Base = Base
...
class NewBoard(Window):
	...
	def __init__(self):
		...
		self.MakeBoard(apollo_interface.PATCH_COMMON + "/new_board/new_Board_Corner_", apollo_interface.PATCH_COMMON + "/new_board/new_Board_Line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage(apollo_interface.PATCH_COMMON + "/new_board/new_Board_Base.png")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class ScrollBarNewDesign(Window):
	...
	class MiddleBar(DragButton):
		...
		def MakeImage(self):
			...
			top.SetParent(self)
			top.LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/large/btn_board_middle_top_01_normal.png")
			top.SetPosition(2, 0)
			top.AddFlag("not_pick")
			top.Show()
			...
			bottom.SetParent(self)
			bottom.LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/large/btn_board_middle_bottom_01_normal.png")
			bottom.AddFlag("not_pick")
			bottom.Show()

			middle = ExpandedImageBox()
			middle.SetParent(self)
			middle.LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/large/btn_board_middle_grip_01_normal.png")
			middle.SetPosition(1, 10)
			middle.AddFlag("not_pick")
			middle.Show()
		...
		def SetSize(self, height):
			height = max(12, height)
			DragButton.SetSize(self, 10, height+15)
			self.bottom.SetPosition(2, height+4)
	...
	def CreateScrollBar(self):
		...
		self.__dictImages['TOP'].LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/vertical_top.png")
		self.__dictImages['CENTER'].LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/vertical_center.png")
		self.__dictImages['BOTTOM'].LoadImage(apollo_interface.PATCH_COMMON + "/scroll_bar/vertical_bottom.png")

		self.__dictImages['TOP'].SetPosition(0, 5)
		self.__dictImages['CENTER'].SetPosition(-10, HEIGHTS['TOP']-5)
	...
	def SetScrollBarSize(self, height):
		...
		self.middleBar.SetPosition(self.MIDDLE_BAR_POS, 0)
		# self.barSlot.SetPosition(0, self.SCROLLBAR_BUTTON_HEIGHT)
		# self.barSlot.SetScale(1.0, (height - self.SCROLLBAR_BUTTON_HEIGHT*16))

		height = max(HEIGHTS['TOP'] + HEIGHTS['BOTTOM'], height)

		self.SetSize(WIDTH, height)

		self.__dictImages['CENTER'].SetScale(3.4, float(10+height - (HEIGHTS['TOP'] + HEIGHTS['BOTTOM'])) / float(HEIGHTS['CENTER']))
		# self.__dictImages['BOTTOM'].SetPosition(-10, height - HEIGHTS['BOTTOM'])
...
class ThinScrollBarNew(ScrollBar):
...
class EditableTextSlot(ImageBox):
	def __init__(self, parent, x, y):
		...
		self.SetPosition(x, y)
		self.LoadImage(apollo_interface.PATCH_COMMON + "/costume/guild_chenar.png")
...
class CheckBox(ImageBox):
	def __init__(self, parent, x, y, event, filename = apollo_interface.PATCH_SPECIAL_1 + "/fail.png"):
		ImageBox.__init__(self)
		self.SetParent(parent)
		self.SetPosition(x, y)
		self.LoadImage(filename)
		...
		image = MakeImageBox(self, apollo_interface.PATCH_SPECIAL_1 + "/ok.png", 0, 0)
		image.AddFlag("not_pick")
		image.SetWindowHorizontalAlignCenter()
		image.SetWindowVerticalAlignCenter()
		image.Hide()
...
class ComboBox_New(Window):
	...
	def __init__(self):
		...
		self.Slots_Grade = InputNew()
		self.Slots_Grade.SetParent(self)
		self.Slots_Grade.AddFlag("not_pick")
		self.Slots_Grade.SetPosition(0,0)
		self.Slots_Grade.MakeTitleBar(105)
		...
		self.test = Button()
		self.test.SetParent(self.Slots_Grade)
		self.test.SetPosition(70,5)
		self.test.SetUpVisual(apollo_interface.PATCH_DROPDW+"btn_01_normal.png")
		self.test.SetOverVisual(apollo_interface.PATCH_DROPDW+"btn_02_hover.png")
		self.test.SetDownVisual(apollo_interface.PATCH_DROPDW+"btn_03_active.png")
		self.test.SetEvent(__mem_func__(self.OnMouseLeftButtonUp))
		self.test.Hide()
	...
	def SetSize(self, width, height):
		Window.SetSize(self, width+30, height)
		self.width = width
		self.height = height
		self.textLine.UpdateRect()
		self.__ArrangeListBox()
	...
	def __ArrangeListBox(self):
		self.listBox.SetPosition(0, self.height + 5+5)
		self.listBox.SetWidth(self.width+5)

	def ShowButton(self,index):
		if index == 1:
			self.test.Show()

	def SizeInput(self,rangeworld):
		self.Slots_Grade.MakeTitleBar(rangeworld)
	...
	def SetCurrentItem(self, text):
		self.textLine.SetText(text)
		self.textLine.SetPosition(-10,4)
...
class BoardNew(Window):
	...
	def __init__(self):
		...
		self.MakeBoard(apollo_interface.PATCH_COMMON+"/board_n/board_corner_", apollo_interface.PATCH_COMMON+"/board_n/board_line_")
		self.MakeBase()
	...
	def MakeBase(self):
		self.Base = ExpandedImageBox()
		self.Base.AddFlag("not_pick")
		self.Base.LoadImage(apollo_interface.PATCH_COMMON+"/board_n/board_base.png")
		self.Base.SetParent(self)
		self.Base.SetPosition(self.CORNER_WIDTH, self.CORNER_HEIGHT)
		self.Base.Show()
...
class Input(Window):
	...
	def __init__(self, layer = "UI"):
		...
		self.__dictImages['LEFT'].LoadImage("d:/ymir work/interface/input/board_left_01_normal.tga")
		self.__dictImages['CENTER'].LoadImage("d:/ymir work/interface/input/board_center_01_normal.tga")
		self.__dictImages['RIGHT'].LoadImage("d:/ymir work/interface/input/board_right_01_normal.tga")
	...
	def SetWidth(self, width):
		...
		self.__dictImages['LEFT'].SetPosition(0, self.HEIGHT - self.__dictImages['LEFT'].GetHeight())
		self.__dictImages['CENTER'].SetPosition(self.__dictImages['LEFT'].GetWidth(), self.HEIGHT - self.__dictImages['CENTER'].GetHeight())
		self.__dictImages['RIGHT'].SetPosition(self.GetWidth() - self.__dictImages['RIGHT'].GetWidth()+12, self.HEIGHT - self.__dictImages['RIGHT'].GetHeight())
		self.__dictImages['CENTER'].SetScale(self.GetWidth(),1.0)

class InputNew(Window):
	...
	def MakeTitleBar(self, width):
		...
		imgLeft.LoadImage(apollo_interface.PATCH_COMMON+"/input/board_left.png")
		imgCenter.LoadImage(apollo_interface.PATCH_COMMON+"/input/center.sub")
		imgRight.LoadImage(apollo_interface.PATCH_COMMON+"/input/right.sub")
	...
	def SetWidth(self, width):
		self.imgCenter.SetRenderingRect(0.0, 0.0, float((width - self.BLOCK_WIDTH*2)) / self.BLOCK_WIDTH_1, 0.0)
		self.imgCenter.SetPosition(0, 2)
		self.imgRight.SetPosition(width - self.BLOCK_WIDTH_1-13, 2)
		self.imgLeft.SetPosition(0,2)

		self.SetSize(width, self.BLOCK_HEIGHT)

class BoardSlot(Window):
	...
	def MakeTitleBar(self, width):
		...
		imgLeft.LoadImage(apollo_interface.PATCH_COMMON + "/expander/left_01_normal.png")
		imgCenter.LoadImage(apollo_interface.PATCH_COMMON + "/expander/center_01_normal.png")
		imgRight.LoadImage(apollo_interface.PATCH_COMMON + "/expander/right_01_normal.png")
	...
	def SetWidth(self, width):
		self.imgCenter.SetRenderingRect(0.0, 0.0, float((width - self.BLOCK_WIDTH*2)) / self.BLOCK_WIDTH_1, 0.0)
		self.imgCenter.SetPosition(0, 0)
		self.imgRight.SetPosition(width - self.BLOCK_WIDTH_1-8, 0)
		self.imgLeft.SetPosition(-4,0)
...
class ButtonNew(Button):
	...
	def MakeTitleBar(self, width):
		...
		imgRight.LoadImage(apollo_interface.PATCH_COMMON + "/costume/sep.png")
	...
	def SetWidth(self, width):
		self.imgCenter.SetRenderingRect(0.0, 0.0, float((width - self.BLOCK_WIDTH*2)) / self.BLOCK_WIDTH_1, 0.0)
		self.imgCenter.SetPosition(0, 2)
		self.imgRight.SetPosition(width - self.BLOCK_WIDTH_1+32, 2)
		self.imgLeft.SetPosition(-8,2)

		self.SetSize(width, self.BLOCK_HEIGHT)

class ThinMenu(Window):
	BLOCK_WIDTH = 272
	BLOCK_HEIGHT = 139
	BLOCK_WIDTH_1 = 1
	...
	def MakeTitleBar(self, width):
		...
		imgLeft.LoadImage(apollo_interface.PATCH_SPECIAL+ "/menu/bg_top.png")
		imgCenter.LoadImage(apollo_interface.PATCH_SPECIAL+ "/menu/bg_center.png")
		imgRight.LoadImage(apollo_interface.PATCH_SPECIAL+ "/menu/bg_bottom.png")
	...
	def SetWidth(self, width):
		self.imgCenter.SetRenderingRect(0.0, float((width - self.BLOCK_WIDTH_1*2)) / self.BLOCK_WIDTH_1, 0.0, 0.0)
		self.imgCenter.SetPosition(0, width+self.BLOCK_HEIGHT)
		self.imgRight.SetPosition(0, width+self.BLOCK_HEIGHT)
		self.imgLeft.SetPosition(0,2)

		self.SetSize(self.BLOCK_WIDTH, width+(self.BLOCK_HEIGHT*2))
...
class CheckBox(Window):
	...
	def OnMouseLeftButtonUp(self):
		import chat
		##chat.AppendChat(chat.CHAT_TYPE_INFO, "hallo")
		if self.checkImage:
			if self.checkImage.IsShow():
				self.checkImage.Hide()
...
if app.ENABLE_ASLAN_LOTTERY:
	class LotteryTicketCheckBox(Window):
		def __init__(self):
			Window.__init__(self)
			
			self.backgroundImage = None
			self.checkNumberImage = None
			self.checkLottoImage = None
			self.checkWinImage = None
		...
		def CreateElements(self):
			...
			self.backgroundImage.SetParent(self)
			self.backgroundImage.AddFlag("not_pick")
			self.backgroundImage.LoadImage("d:/ymir work/ui/lottery/number_slot.sub")
			self.backgroundImage.Show()
			
			self.checkNumberImage = ImageBox()
			self.checkNumberImage.SetParent(self)
			self.checkNumberImage.AddFlag("not_pick")
			self.checkNumberImage.LoadImage("d:/ymir work/ui/lottery/number_slot_checked.sub")
			self.checkNumberImage.Hide()
	
			self.checkLottoImage = ImageBox()
			self.checkLottoImage.SetParent(self)
			self.checkLottoImage.AddFlag("not_pick")
			self.checkLottoImage.LoadImage("d:/ymir work/ui/lottery/number_slot_checked_lotto.sub")
			self.checkLottoImage.Hide()
			
			self.checkWinImage = ImageBox()
			self.checkWinImage.SetParent(self)
			self.checkWinImage.AddFlag("not_pick")
			self.checkWinImage.LoadImage("d:/ymir work/ui/lottery/number_slot_checked_win.sub")
			self.checkWinImage.Hide()
			
			self.number = TextLine()
			self.number.SetParent(self)
			self.number.SetFontName(localeInfo.UI_DEF_FONT_LARGE)
			self.number.SetPosition((self.backgroundImage.GetWidth() / 2) - ( self.number.GetTextSize()[0] / 2), (self.backgroundImage.GetHeight() / 2) - (self.number.GetTextSize()[1] / 2) - 1)
			self.number.Show()
		...
		def SetCheckNumber(self, flag):
			if flag:
				self.checkNumberImage.Show()
			else:
				self.checkNumberImage.Hide()
		
		def IsCheckNumber(self):
			if self.checkNumberImage:
				return self.checkNumberImage.IsShow()
				
			return False
			
		def SetLottoNumber(self, flag):
			if flag:
				self.checkLottoImage.Show()
			else:
				self.checkLottoImage.Hide()
		
		def IsLottoNumber(self):
			if self.checkLottoImage:
				return self.checkLottoImage.IsShow()
				
			return False
			
		def SetWinNumber(self, flag):
			if flag:
				self.checkWinImage.Show()
			else:
				self.checkWinImage.Hide()
		
		def IsWinNumber(self):
			if self.checkWinImage:
				return self.checkWinImage.IsShow()
				
			return False

						
	class LotteryNewTicketCheckBox(Window):
		def __init__(self):
			Window.__init__(self)
			...
			self.eventFunc = { "ON_CHECK" : None, "ON_UNCKECK" : None, "mouse_over_in" : None, "mouse_over_out" : None}
			self.eventArgs = { "ON_CHECK" : None, "ON_UNCKECK" : None, "mouse_over_in" : None, "mouse_over_out" : None}
		...
		def CreateElements(self):
			...
			self.backgroundHoverImage = ImageBox()
			self.backgroundHoverImage.SetParent(self)
			self.backgroundHoverImage.AddFlag("not_pick")
			self.backgroundHoverImage.LoadImage("d:/ymir work/ui/lottery/number_slot_hover.sub")
			self.backgroundHoverImage.Hide()
			
			self.checkImage = ImageBox()
			self.checkImage.SetParent(self)
			self.checkImage.AddFlag("not_pick")
			self.checkImage.LoadImage("d:/ymir work/ui/lottery/number_slot_checked.sub")
			self.checkImage.Hide()
	
			#wndMgr.SetOverVisual(self.hWnd, "d:/ymir work/ui/lottery/number_slot_checked.sub")
		...
		def OnMouseOverIn(self):
			self.backgroundHoverImage.Show()
		...
		def OnMouseLeftButtonUp(self):
			...
			if self.checkImage:
				...
				if self.checkImage.IsShow():
					self.checkImage.Hide()
					# for i in xrange(0, len(constInfo.lottery_new_ticket_numbers)):
						# if self.num_value == constInfo.lottery_new_ticket_numbers[i]:
					constInfo.lottery_new_ticket_numbers.remove(int(self.num_value))

					if self.eventFunc["ON_UNCKECK"]:
						apply(self.eventFunc["ON_UNCKECK"], self.eventArgs["ON_UNCKECK"])
				else:
					if len(constInfo.lottery_new_ticket_numbers) >= 4:
						chat.AppendChat(chat.CHAT_TYPE_INFO, "Du hast bereits 4 Zahlen ausgewählt")
						return

					self.checkImage.Show()
					constInfo.lottery_new_ticket_numbers.append(int(self.num_value))

					if self.eventFunc["ON_CHECK"]:
						apply(self.eventFunc["ON_CHECK"], self.eventArgs["ON_CHECK"])

				chat.AppendChat(chat.CHAT_TYPE_INFO, "%s" % constInfo.lottery_new_ticket_numbers)
...
class PythonScriptLoader(object):
	...
	if app.ENABLE_ASLAN_LOTTERY:
		LOTTERY_CHECKBOX_KEY_LIST = ( "number", )
	...
	def LoadChildren(self, parent, dicChildren):
		...
		for ElementValue in ChildrenList:
			...
			if Type == "window":
			...
			elif Type == "board_with_titlebar_shop":
				parent.Children[Index] = BoardWithTitleBarNew("d:/ymir work/ui/pattern/shop_board/shop_board")
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoardWithTitleBar(parent.Children[Index], ElementValue, parent)
			...
			elif Type == "thinboard_circle2":
				parent.Children[Index] = ThinBoardCircleNoAlpha()
				parent.Children[Index].SetParent(parent)
				self.LoadElementThinBoardCircle(parent.Children[Index], ElementValue, parent)
				
			elif Type == "thinboard_round":
				parent.Children[Index] = ThinBoardRoundCorner()
				parent.Children[Index].SetParent(parent)
				self.LoadElementThinBoard(parent.Children[Index], ElementValue, parent)
			...
			elif Type == "thin_scrollbar_new":
				parent.Children[Index] = ThinScrollBarNew()
				parent.Children[Index].SetParent(parent)
				self.LoadElementScrollBar(parent.Children[Index], ElementValue, parent)
				...
				parent.Children[Index] = MenuBoard()
				parent.Children[Index].SetParent(parent)
				...
				parent.Children[Index] = BoardNew()
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoardNew(parent.Children[Index], ElementValue, parent)

			elif Type == "inputnew":
				parent.Children[Index] = InputNew()
				parent.Children[Index].SetParent(parent)
				self.LoadElementInputNew(parent.Children[Index], ElementValue, parent)

			elif Type == "boardslot":
				parent.Children[Index] = BoardSlot()
				parent.Children[Index].SetParent(parent)
				self.LoadElementBoardSlot(parent.Children[Index], ElementValue, parent)

			elif Type == "thinmenu":
				parent.Children[Index] = ThinMenu()
				parent.Children[Index].SetParent(parent)
				self.LoadElementThinMenu(parent.Children[Index], ElementValue, parent)
			
			elif Type == "lottery_ticket_checkbox" and app.ENABLE_ASLAN_LOTTERY:
				parent.Children[Index] = LotteryTicketCheckBox()
				parent.Children[Index].SetParent(parent)
				self.LoadElementLotteryCheckbox(parent.Children[Index], ElementValue, parent)
				
			elif Type == "lottery_new_ticket_checkbox" and app.ENABLE_ASLAN_LOTTERY:
				parent.Children[Index] = LotteryNewTicketCheckBox()
				parent.Children[Index].SetParent(parent)
				self.LoadElementLotteryCheckbox(parent.Children[Index], ElementValue, parent)
	...
	def LoadElementHorizontalSeparator(self, window, value, parentWindow):
		...
		if False == self.CheckKeyList(value["name"], value, self.HORIZONTAL_SEP_KEY_LIST):
			return False
	...
	def LoadElementText(self, window, value, parentWindow):
		...
		if value.has_key("enable_enter"):
			if value["enable_enter"]:
			
				window.SetEnterToken(True)
		if TRUE == value.has_key("text"):
			window.SetText(value["text"])
...
## MultiTextLine
	...
	def LoadElementInputNew(self, window, value, parentWindow):
		if FALSE == self.CheckKeyList(value["name"], value, self.TITLE_BAR_KEY_LIST):
			return FALSE
	...
	def LoadElementBoardNew(self, window, value, parentWindow):
	...
	def LoadElementBoardSlot(self, window, value, parentWindow):
		if FALSE == self.CheckKeyList(value["name"], value, self.TITLE_BAR_KEY_LIST):
			return FALSE
	...
	def LoadElementThinMenu(self, window, value, parentWindow):
		if FALSE == self.CheckKeyList(value["name"], value, self.TITLE_BAR_KEY_LIST):
			return FALSE
	...
	if app.ENABLE_ASLAN_LOTTERY:
		def LoadElementLotteryCheckbox(self, window, value, parentWindow):
			if False == self.CheckKeyList(value["name"], value, self.LOTTERY_CHECKBOX_KEY_LIST):
				return False
			...
			if value.has_key("number"):
				window.SetNumber(str(value["number"]))
...
def CookiesScaleBox(parent, name, x, y,xscale,yscale):
	CookiesScale = ImageBox()
	CookiesScale.SetParent(parent)
	CookiesScale.LoadImage(name)
	if CookiesScale.GetHeight() == 96:
		CookiesScale.SetScale(xscale-0.2, yscale-0.2)
	else:
		CookiesScale.SetScale(xscale, yscale)
	if CookiesScale.GetHeight() == 32:
		CookiesScale.SetPosition(x, y+15)
	elif CookiesScale.GetHeight() == 64:
		CookiesScale.SetPosition(x, y+4)
	else:
		CookiesScale.SetPosition(x, y)
	CookiesScale.Show()
	return CookiesScale
...
class ScrollBarShop(Window):
	...
	class MiddleBar(DragButton):
		...
		def MakeImage(self):
			top = ImageBox()
			top.SetParent(self)
			top.LoadImage("d:/ymir work/ui/pattern/ScrollBar_Top_shop.tga")
			top.SetPosition(0, 0)
			top.AddFlag("not_pick")
			top.Show()
			bottom = ImageBox()
			bottom.SetParent(self)
			bottom.LoadImage("d:/ymir work/ui/pattern/ScrollBar_Bottom_shop.tga")
			bottom.AddFlag("not_pick")
			bottom.Show()

			middle = ExpandedImageBox()
			middle.SetParent(self)
			middle.LoadImage("d:/ymir work/ui/pattern/ScrollBar_Middle_shop.tga")
			middle.SetPosition(0, 4)
			middle.AddFlag("not_pick")
			middle.Show()
	...
	def CreateScrollBar(self):
		...
		upButton.SetParent(self)
		upButton.SetEvent(__mem_func__(self.OnUp))
		upButton.SetUpVisual("d:/ymir work/ui/public/scrollbar_up_button_01_shop.sub")
		upButton.SetOverVisual("d:/ymir work/ui/public/scrollbar_up_button_02_shop.sub")
		upButton.SetDownVisual("d:/ymir work/ui/public/scrollbar_up_button_03_shop.sub")
		upButton.Show()

		downButton = Button()
		downButton.SetParent(self)
		downButton.SetEvent(__mem_func__(self.OnDown))
		downButton.SetUpVisual("d:/ymir work/ui/public/scrollbar_down_button_01_shop.sub")
		downButton.SetOverVisual("d:/ymir work/ui/public/scrollbar_down_button_02_shop.sub")
		downButton.SetDownVisual("d:/ymir work/ui/public/scrollbar_down_button_03_shop.sub")
		downButton.Show()
...
class SearchWindow(BoardWithTitleBarNew):
	def GetItemListSortLower(self):
		itemList = []

		for index in range(item.GetProtoSize()):
			vnum = item.GetVnumFromProtoIndex(index)
			item.SelectItem(vnum)
			itemType = item.GetItemType()
			if item.GetItemName():
				itemList.append(self.SearchObject(item.GetItemName(), vnum, "item"))
		return itemList
		
	def SearchObject(self, name, vnum, type):
		return name+'_'+str(vnum)
		
	def GetMobListSortLower(self):
		mobList = []
		for index in range(nonplayer.GetProtoSize()):
			vnum = nonplayer.GetVnumFromProtoIndex(index)
			if nonplayer.GetMonsterName(vnum):
				mobList.append(self.SearchObject(nonplayer.GetMonsterName(vnum), vnum, "mob"))
		return mobList

	def Search(self, search):
		if len(search) < MIN_SEARCHTEXT_LEN and len(search) != 0:
			return
		search = search.lower()
		x = time.clock()
		self.foundItems = []
		# Search by filter
		if self.curFilter == "search":
			pass
		else:
			# Search Items
			if self.curFilter == "all" or self.curFilter == "item": 
				for itemGroup in self.itemList:
					if (self.startsWithSearchMode and itemGroup.GetLowerText().startswith(search)) or (not self.startsWithSearchMode and search in itemGroup.GetLowerText()):
						item.SelectItem(itemGroup.GetId())
						self.foundItems.append(itemGroup)
			# Search Mobs 
			if self.curFilter == "all" or self.curFilter == "mob": 
				for mobGroup in self.mobList:
					if (self.startsWithSearchMode and mobGroup.GetLowerText().startswith(search)) or (not self.startsWithSearchMode and search in mobGroup.GetLowerText()):
						self.foundItems.append(mobGroup)
		
		self.foundItems = sorted(self.foundItems, key=lambda foundItem: foundItem.GetLowerText())
		self.SetCurPage(0)

	def OnUpdate(self):
		if self.lastSearchTime < time.clock():
			searchText = self.elements['editline']['search'].GetText()
			if self.lastSearchText != searchText:
				self.lastSearchText = searchText
				self.Search(searchText)
			self.lastSearchTime = time.clock() + SEARCH_UPDATE_TIME
		if self.elements['editline']['search'].GetText():
			self.elements['button']['clear_search_text'].Show()
		else:
			self.elements['button']['clear_search_text'].Hide()